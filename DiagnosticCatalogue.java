        ...
        try {
            ...
            for (int i = 0; i < testList.getLength(); i++) {
                ...
                if (testNode.getNodeType() == Node.ELEMENT_NODE) {
                    ...
                    currentTest.id = testElement.getAttribute("id");
                    currentTest.name = testElement.getAttribute("name");
                    currentTest.type = testElement.getAttribute("type");

                    if (currentTest.isRaw()) {
                        currentTest.sampleType = testElement.getAttribute("sample_type");
                        currentTest.cost = Integer.parseInt(testElement.getAttribute("collection_cost"));
                    } else {
                        NodeList inputNodes = testElement.getElementsByTagName("input");
                        for (int j = 0; j < inputNodes.getLength(); j++) {
                            Node inputNode = inputNodes.item(j);
                            if (inputNode.getNodeType() == Node.ELEMENT_NODE) {
                                Element inputElement = (Element) inputNode;
                                currentTest.inputs.add(inputElement.getAttribute("ref"));
                            }
                        }
                    }
                    tests.put(currentTest.id, currentTest);
                }
            }
        } catch (Exception e) {
            e.printStackTrace();
            ...
            if (t.isDerived()) {
                ...
                for (String inputId : t.inputs) {
                    ...
                    if (inputTest != null && inputTest.isRaw()) {
                        if (inputTest.sampleType != null && !inputTest.sampleType.equalsIgnoreCase("NONE")) {
                            distinctSamples.add(inputTest.sampleType.toUpperCase());
                        }
                    }
                }
                t.cost = distinctSamples.size();
