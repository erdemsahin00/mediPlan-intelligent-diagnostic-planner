...
public class MediPlanDP {
    ...
    private Set<String> computeSamples(String testId, int depth) {
        ...
        if (t.isRaw()) {
            if (t.sampleType != null && !t.sampleType.equalsIgnoreCase("NONE")) {
                result.add(t.sampleType.toUpperCase());
            ...
            for (String depId : t.inputs) {
                result.addAll(computeSamples(depId, depth + 1));
            }
    ...
    public void computeCostBottomUp(List<String> targetIds) {
        ...
        for (String testId : topoOrder) {
            ...
            List<String> depCosts = new ArrayList<>();
            if (t.isDerived() && t.cost > 0) {
                depCosts.add(String.valueOf(t.cost));
            }

            for (String dep : t.inputs) {
                int dCost = costTable.getOrDefault(dep, 0);
                total += dCost;
                depCosts.add(String.valueOf(dCost));
            }
            costTable.put(testId, total);

            String formula;
            if (t.isRaw()) {
                formula = "collection_cost = " + t.cost;
            } else {
                String sumStr = String.join(" + ", depCosts);
                if (sumStr.isEmpty()) sumStr = "0";

                if (t.isDerived()) {
                    formula = "processing_cost = " + sumStr;
                } else {
                    formula = "cost = " + sumStr;
                }
            }

            System.out.printf("Computing %-28s %-32s total: %d%n", testId + ":", formula, total);
        }

        System.out.println();
        System.out.println("Results:");
        for (String target : targetIds) {
            System.out.printf("  %-27s naive hospital cost: %d%n", target, costTable.getOrDefault(target, 0));
        }
    ...
    public List<String> topologicalSort(Set<String> reachable) {
        ...
        for (String node : reachable) {
            if (!visited.contains(node)) {
                dfsFinish(node, reachable, visited, finishStack);
            }
        }

        List<String> sorted = new ArrayList<>();
        while (!finishStack.isEmpty()) {
            sorted.add(finishStack.pollLast());
        }
    ...
    private void dfsFinish(String testId, Set<String> reachable,
        ...
        if (t != null) {
            for (String dep : t.inputs) {
                if (reachable.contains(dep) && !visited.contains(dep)) {
                    dfsFinish(dep, reachable, visited, finishStack);
                }
            }
        }
    ...
    public int buildDiagnosticPlan(String targetId) {
        ...
        for (String s : samples) {
            procs.add(procedureLabel(s));
        }
        Collections.sort(procs);

        if (!procs.isEmpty()) {
            System.out.print("  Procedures:    ");
            System.out.println(String.join(", ", procs));
        }
        System.out.println();
        ...
        for (String id : execOrder) {
            ...
            System.out.printf("  [%2d]  %-28s %-13s added cost:%3d%s%n",
                    step++, id, typeStr, t.cost, annotation);
            optCost += t.cost;
        }

        System.out.println();
        System.out.printf("Naive hospital cost:     %d%n", costTable.getOrDefault(targetId, 0));
        System.out.printf("Optimized hospital cost: %d%n", optCost);
